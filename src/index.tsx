...
const CreateMapFn = ({
  ...
  getSVGProps,
}: USAProps) => (key: keyof StatesListRendererMap) => {
  let Renderer = RENDERERS_MAP[key];
  let svg_props = getSVGProps != null ? getSVGProps(key) : {};

  if (!Renderer) return null;

  if (HOC) {
    return (
      <HOC renderer={Renderer} svg_props={svg_props} key={key} abbr={key} />
    );
  }

  return <Renderer svg_props={svg_props} key={key} />;
};

let defaultFramesStroke = "#999";
let defaultFramesStrokeWidth = 2;
...
export function USA(props: USAProps) {
  ...
  let framesStroke = _framesStroke || defaultFramesStroke;
  let framesStrokeWidth = _framesStrokeWidth || defaultFramesStrokeWidth;

  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 959 593">
      {SVGFilters?.map((SVGFilterComponent, key) => (
        <SVGFilterComponent key={key} />
      ))}
...
export const RENDERERS_MAP: StatesListRendererMap = {
  AK: ({ svg_props }) => (
    <path
      {...svg_props}
      id="AK"
